...
expclim[which()]
tchill<-5
expsites<-unique(expclim$site)
expclim$chday<-NA
expclim$chillyear<-NA
for (i in 1:dim(expclim)[1]){
  if(!is.na(expclim$soiltemp1_mean[i])){
    expclim[expclim$soiltemp1_mean[i]<tchill,]$chday[i]<-1
    sitedat[which(sitedat$soiltemp1_mean[i]>=tchill),]$chday[i]<-0
    sitedat$chillyear<-sitedat$year
  years<-unique(expclim[expclim$site==expclim$site[i],]$year)
