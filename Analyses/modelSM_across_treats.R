...
#dir<-"/Users/aileneettinger/git/radcliffe/Analyses"
...
#may want to use "treats_detail.csv" instead
### (A) Data clean-up ----------------------------------------------------------------------
...
colnames(expsiteinfo)[38]<-"preciptreat_-1"
...
for(j in 1:length(sites)){
  ...
  info<-subset(expsiteinfo,expsiteinfo$DatasetID==sites[j])
...
#FYI, this is also in "treats.detail.csv"
...
#Ailene: I got error message "Error in if (zero_range(from) || zero_range(to)) { : 
#missing value where TRUE/FALSE needed" so was not abke to see any of thesefigures
...
#Ailene: I agree with this reasoning. the only thing to consider is year effects- there may be variation across years (either/both within and among sites, due to annual weather variation as well as other differences that may have been imposed in the experiment itself)
...
#no need to do this- bace has been fixed!also, sitenames have changed so bace now="exp01"
...
moddat<-dat[,which(colnames(dat) %in% c("site","block","plot","year","doy","airtemp_min","airtemp_max",
                                        ...
                                        "year.frac","Pval"))]
dim(moddat[complete.cases(moddat),]) #nrow= (Ailene: I get 8428)
dim(moddat[which(!is.na(moddat$airtemp_max) & !is.na(moddat$soiltemp1_max)),]) #nrow=0 (Ailene: I get 8428)
...
moddat<-dat[,which(colnames(dat) %in% c("site","block","plot","year","doy","airtemp_min","airtemp_max",
                                        ...
                                        "year.frac","Pval"))]
dim(moddat[complete.cases(moddat),]) #nrow= 0 (Ailene: still get 8428)
dim(moddat[which(!is.na(moddat$airtemp_max) & !is.na(moddat$soiltemp1_max)),]) #nrow=0 (8428)
...
#Ailene: i get nwrow=28944, with the following sites: "exp01" "exp12" "exp09"
...
#Ailene: I don't get this error, but my data also includes three sites when i run this, so the model is inappropriate for other reasons (missing site as a random effect)
#Ailene: also, for blocked data, it makes sense to use block (and perhaps plot as well, to account for repeated measures over time, but plot should be nested within block) as the random effect. plot is the level at which the data are measured- i.e. there is one temperature sensor per plot
...
#Ailene:This is true for all sites except farnsworth/exp08), which ONLY measured soiltempmean (not min and max)
...
#well, i disagree wtih CJ on the interpretability in this case (you can still get a slope and intercept for each year), but either way, i don't think year should be a fixed effect
...
#Ailene: exactly! so mod2 is not a good structure to use
summary(mod2)
plot(mod2)
...
#Ailene: No, you cannot compare likelihood (or AIC) unless the datasets are the same across models
...
#Remove chuine
df.c<-df.b[which(df.b$site!="exp02"),] #196633 rows 16 Oct 16: remove "chuine"
...
#Remove bace
df.f<-df.e[which(df.e$site!="exp01"),] #92101 - didn't change because bace doesn't have air temp.
...
el<-df.g[which(df.g$site=="exp07"),] #exp07 = ellison
m<-mean(el$soilmois1)
s<-sd(el$soilmois1)
df.h<-df.g[-which(df.g$site=="exp07" & df.g$soilmois1>(m+(6*s))),] #90319
#Double-checking
summary(df.h) #looks good
el<-df.h[which(df.h$site=="exp07"),]
...
### UPDATE 16Oct16 ### : mod10.x is better than mod10.u now!! (more/less data??)
...
### UPDATE 16Oct16 ###: now of the 2-variable models, mod10.p is best (airtemp_mean + soiltemp_max)
...
#mod10.j diagnostic plots: ### UPDATE 16Oct16: SEE mod 10.p diagnostic plots below plots for mod10.j###
par(mfrow=c(1,2))
...
#Same as above, with mod10.p:
par(mfrow=c(1,2))
qqnorm(resid(mod10.p))
plot(fitted(mod10.p),resid(mod10.p),xlab="Fitted",ylab="Resid");abline(0,0)

#Do I want to try out interactions now, even though they didn't work before? Use mod10.j as a base model:
mod10.p<-lmer(soilmois1~airtemp_mean+ soiltemp1_max +
                as.factor(season)+doy+ #controlling for temporal variation
                (1|site)+(1|year), data=dat, REML=FALSE)  #random effects
mod10.y<-lmer(soilmois1~airtemp_mean+soiltemp1_max+airtemp_mean*soiltemp1_max+
                as.factor(season)+doy+ #controlling for temporal variation
                (1|site)+(1|year), data=dat, REML=FALSE)  #random effects
summary(mod10.y) #all t values have a largish absolute value
anova(mod10.j,mod10.y) #mod10.y is way better!
...
qqnorm(resid(mod10.p))
plot(fitted(mod10.p),resid(mod10.p),xlab="Fitted",ylab="Resid");abline(0,0)
#There is something weird going on with the residual plot for mod10.y with airtemp_mean*soiltemp1_max
#So: stick with mod10.p/


#IN SUMMARY (before 16Oct16 update):
...
#-I made models with every combination of fixed predictors (no interactions, all had the same random 
#effects structure, all had control variables for doy and season) and compared them via AIC. 
#best overall modelaccording to AIC: mod10.x (airtemp min/max, soiltemp min/max), AIC = -296021;
#second-best: mod10.u (airtemp min/max, soiltemp mean), AIC=--295997.9
#-Chose mod10.p (5th best model, AIC = -295977.4) because it was very close to the others and only included one of each type of var (airtemp, soiltemp)
...
#Hi Miriam:
#A few notes on your models:
#First, thank you so much for doing all this! You have done lots of model exploration and I really appreciate that!
#Second, we need to keep in mind the main question underlying these analyses, which, in my mind, 
#is: How do experimental treatments affect soil moisture? This question is important
#because we are trying to address what secondary effects there are
#Toward that end, i think it is worth having a model with the "target warming" (i.e. the warming treatment)
#as an explanatory variable. 
#Also, i wanted to let you know that i have been using "Above ground temeprature" (which incluldes surface temperature and canopy temperature, in addition to air temperature)
#This allows you to use more of the data that we have
#You had asked about the random effects structure, and i used year nested within site becuase
#i wanted to account for the nonindepence in the data, and i didn't think it was fair to treat, for example, 2009 at BACE the same as 2009 at Duke, since the weather and other environmental variables that vary by year are unlikely to be the same in these two sites, even though the year is the same
#As a side note, it might be interesting to include year a little bit differently as a fixed effect, rather than a random effect
#In this case, it would be "years since the start of the experiment" so that rather than the actual dated year, we would have year 1,2,3, etc of the experiment. 
#It seems possible that soils may dry out over time in these expimerments and it might be interesting to test this idea.
#
