...
extract.gldas.region <- function(outfolder, start_date, end_date, site_id=NULL, lat.in=NULL, lon.in=NULL, ext=NULL,
                                 vars,
  ...
  if(start_year < 1980){ 
    warning("GLDAS 2.1 starts in 2000 A.D.; resetting start_date to 2000-01-01")
    start_date <- as.POSIXlt("2000-01-01", tz = "UTC")
    start_year <- lubridate::year(start_date)
  }
  ...
  if(is.null(site_id) & is.null(ext)){ 
    site_id <- paste0("site", str_pad(1:length(lat.in), nchar(length(lat.in)), pad="0"))
  } else {
    site_id <- "site1"
  }
  ...
  # GLDAS 2.1 Path for example file:
  # https://hydro1.gesdisc.eosdis.nasa.gov/opendap/GLDAS/GLDAS_NOAH025_3H.2.1/2017/090/GLDAS_NOAH025_3H.A20170331.0000.021.nc4
