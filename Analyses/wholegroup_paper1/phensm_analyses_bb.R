...
#Need to summarize soil moisture and air temperature by plot, seasonally and annually
#Then merge this in to climate data

#start by aggregating observed above-ground min and max and soil moisture by plot and year to get annual values
ag_max_plot<-aggregate(expclim2$agtemp_max, 
                       by=list(expclim2$site,expclim2$block,expclim2$plot,expclim2$target,expclim2$preciptreat_amt,expclim2$year), 
                       FUN=mean,na.rm=TRUE)
ag_min_plot<-aggregate(expclim2$agtemp_min, 
                       by=list(expclim2$site,expclim2$block,expclim2$plot,expclim2$target,expclim2$preciptreat_amt,expclim2$year), 
                       FUN=mean,na.rm=TRUE)
soilmois_plot<-aggregate(expclim2$soilmois1, 
                       by=list(expclim2$site,expclim2$block,expclim2$plot,expclim2$target,expclim2$preciptreat_amt,expclim2$year), 
                       FUN=mean,na.rm=TRUE)
...
#Make seasonal summaries as well: 
#Jan-Mar
janmar<-expclim2[as.numeric(expclim2$doy)<90,]
soilmois_janmar<-aggregate(janmar$soilmois1, 
                           by=list(janmar$site,janmar$block,janmar$plot,janmar$target,janmar$preciptreat_amt,janmar$year), 
                           FUN=mean,na.rm=TRUE)
ag_min_janmar<-aggregate(janmar$agtemp_min, 
                         by=list(janmar$site,janmar$block,janmar$plot,janmar$target,janmar$preciptreat_amt,janmar$year), 
                         FUN=mean,na.rm=TRUE)
ag_max_janmar<-aggregate(janmar$agtemp_max, 
                         by=list(janmar$site,janmar$block,janmar$plot,janmar$target,janmar$preciptreat_amt,janmar$year), 
                         FUN=mean,na.rm=TRUE)
#combine to one datafreame
tempsm_janmar<-cbind(ag_max_janmar,ag_min_janmar$x,soilmois_janmar$x)
colnames(tempsm_janmar)<-c("site","block","plot","target","preciptreat_amt","year","agtmax_janmar","agtmin_janmar","soilmois_janmar")
tempsm_janmar<-tempsm_janmar[order(tempsm_janmar$site,tempsm_janmar$block,tempsm_janmar$plot,tempsm_janmar$year),]

#Apr-Jun
aprjun<-expclim2[as.numeric(expclim2$doy)>=90 & as.numeric(expclim2$doy)<181,]
soilmois_aprjun<-aggregate(aprjun$soilmois1, 
                           by=list(aprjun$site,aprjun$block,aprjun$plot,aprjun$target,aprjun$preciptreat_amt,aprjun$year), 
                           FUN=mean,na.rm=TRUE)
ag_min_aprjun<-aggregate(aprjun$agtemp_min, 
                         by=list(aprjun$site,aprjun$block,aprjun$plot,aprjun$target,aprjun$preciptreat_amt,aprjun$year), 
                         FUN=mean,na.rm=TRUE)
ag_max_aprjun<-aggregate(aprjun$agtemp_max, 
                         by=list(aprjun$site,aprjun$block,aprjun$plot,aprjun$target,aprjun$preciptreat_amt,aprjun$year), 
                         FUN=mean,na.rm=TRUE)
#combine to one datafreame
tempsm_aprjun<-cbind(ag_max_aprjun,ag_min_aprjun$x,soilmois_aprjun$x)
colnames(tempsm_aprjun)<-c("site","block","plot","target","preciptreat_amt","year","agtmax_aprjun","agtmin_aprjun","soilmois_aprjun")
tempsm_aprjun<-tempsm_aprjun[order(tempsm_aprjun$site,tempsm_aprjun$block,tempsm_aprjun$plot,tempsm_aprjun$year),]
...
expgdd_subs<-subset(expgdd_subs,select=c(site,block, plot,year,target,preciptreat_amt,agtmax,agtmin,sm,doy,genus.species,event,cumgdd_air,agtmax_janmar,agtmin_janmar,soilmois_janmar,agtmax_aprjun,agtmin_aprjun,soilmois_aprjun))
...
smbbdmod<-lmer(doy~agtmax_janmar*soilmois_janmar + (1|genus.species)+ (1|site/year)+ (1|year), REML=FALSE, data=expgdd_bbd)
summary(smbbdmod)
...
#abline(a=fixef(smbbdmod_targt)[1],b=fixef(smbbdmod_targt)[2], lwd=2)
#abline(a=fixef(smbbdmod_agtmax)[1],b=fixef(smbbdmod_agtmax)[2], lty=2,lwd=2)#measured temp mod
