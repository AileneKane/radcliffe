...
\clearpage
 \begin{figure}[h]
\centering
 \includegraphics{/Users/aileneettinger/Documents/GitHub/radcliffe/Analyses/soilmoisture/figures/smtempvstargtemppreciptreat_lineslmerALL.pdf}
...
\begin{figure}[h]
\centering
 \includegraphics{/Users/aileneettinger/Documents/GitHub/radcliffe/Analyses/soilmoisture/figures/m5bbdlodffd.pdf}
...
\begin{figure}[h]
\centering
 \includegraphics{/Users/aileneettinger/Documents/GitHub/radcliffe/Analyses/soilmoisture/figures/soilmoishist_mn.pdf}
...
%\item National climate assessment for western US (https://nca2014.globalchange.gov/highlights/report-findings/future-climate/graphics/projected-changes-soil-moisture#image-tab2)
...
% It would help me work through an outline and then return to the text ... To get that going I have just jotted down below topics I think we should consider covering in the paper. This is NOT well organized currently. 
There are several ways to open the paper (e.g., the one below, or focus on soil moisture/mutiple drivers) ... not sure which is best. 

\begin{enumerate}
\begin{enumerate} Intro paragraphs
\item Phenology most reported bio indicator of climate change etc.
\item How plant phenology works (see current text)
\item Decades of focus on warm temperatures has left significant questions:
\begin{enumerate}
\item Experiments differ from observations
\item Non-linearities in some places (Tibet, Europe etc.)
\end{enumerate}
\item Somehow loop to moisture ...
\item Need to cover re: moisture
\begin{enumerate}
\item {\bf Temp and moisture are inherently linked, that's climate} *I think we should cover this more strongly than we do. 
\item Forecasts of moisture (do we need to mention snow?)
\item Precip experiments (maybe? And precip x warming?)
\item Phenology and moisture ... how many studies outside semi-arid/arid and grassland sites?
\end{enumerate}
\item Temp and precip/soil moisture are (not sure we need this, maybe an alternative opening? Anyway, I had jotted it down, so keeping for now)
\begin{enumerate}
\item Shifting already, and expected to see more changes due to climate change
\item Well studied experimentally
\item But often considered in isolation in most other studies. Esp. for phenology?
\item Yet, climatically they are strongly intertwined.
\end{enumerate}
\end{enumerate}

\item Results & Conclusions
\begin{enumerate}
\item Relationship between temp and moisture (in control and all plots)
\item Overall effects of temp and moisture on phenology, by stage
\item By species
\item Biome effects? (How many studies for each main model? Add n to plots, and check moisture x temp for control vs. all plots for subset of data in each model)
\item Depth of sensors and/or planted versus unplanted results
\item Forecasts
\item Are we novel in showing moisture effects in temperate sites? 
\end{enumerate}
\item Miscellaneous: Do we need to cover?
\begin{enumerate}
\item Why no long-term data included?
\item Evidence of non-linearities in responses?
\end{enumerate}
\end{enumerate}
