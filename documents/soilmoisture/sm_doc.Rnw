...
\title{Some new title: Drier soils delay plant phenology across temperate forest and grassland systems} %24Jul2022EMW -- Not committed to this; just tossing it out there, one obvious alternative is 'wetter soils advance plant phenology ...' but I think maybe this title, which focuses more on moisture as a limiting factor is better? 
...
Previous meta-analyses of phenology responses to climate change have focused largely on temperature as a driver of observed shifts. However, soil moisture is also affected by climate change and likely to alter biological responses. Here we synthesize microclimate and phenology data from climate change experiments in temperate forests and grasslands to quantify how soil moisture interacts with temperature to affect plant phenology. %24Jul2022EMW -- previous bit of abstract is great!
We find that phenology (budburst, leafout and flowering) delays in drier soils, with the largest (?) delays seen in budburst (0.42 days per percent reduction in soil VWC). While effects of soil moisture were much smaller than for temperature (XX versus XX in standardized units), with interactive effects of temperature x moisture even smaller (XX), there was high variability in the response across species. Forecasting shifts in soil moisture with warming, we find that soil moisture declines of 10\% would have important effects on the phenology of some species, potentially muting advances with warming alone. Our results show that soil moisture plays an important role in the phenology of temperate systems and may be critical for accurate projections. Quantifying phenological sensitivity to changes in soil moisture will therefore likely improve forecasts of shifts in phenology with future climate change at the fine spatial scales relevant for management and conservation.  %24Jul2022EMW -- I really like this last sentence!

% The magnitude of effects we quantify suggest that climate change-induced shifts in soil moisture will generally play a small role in altering future phenology, compared to shifts in temperature, both because of the strong sensitivity of plant phenology to temperature and because of the large magnitude of projected shifts in temperature, compared to shifts in soil moisture. Nonetheless, although effects of soil moisture are comparably small across all species in our analysis, sensitivity to soil moisture varied dramatically by species, and soil moisture levels differed by site and among years. Thus, soil moisture is likely to be an important factor affecting phenological shifts with climate change for some species, in some locations and years. 


\newpage
\section* {INTRODUCTION} %24Jul2022EMW -- generally very nice!
...
%24Jul2022EMW -- I think you can sneak in GDD here where you suggest but I would do it very quickly ... something like 'Forcing effects have been considered so dominant that many models use only forcing to predict phenology, including common models of 'growing degree days,' where phenological events are triggered after a certain thermal sum is reached. 
\par Effects of altered precipitation and soil moisture on phenology have received less attention, but are likely to be important drivers of plant phenology. For example, budburst, flowering, and leaf drop are affected by tree water status in dry ecosystems \citep[e.g., ][]{essiamah1986,reich1984, van1993}. Budburst can be slowed by water stress through inhibiting cell elongation \citep{essiamah1986}, and growing season start may be delayed by drought in grasslands \cite{cui2017}. Flowering phenology, on the other hand, can be advanced by drought conditions \citep{hamann2018}. When effects of soil moisture on phenology have been quantified, this has occurred largely in arid and grassland or meadow ecosystems (e.g., Cleverly 2016, Tao et al 2019, Ganjurjac et al 2020); its role in other ecosystem types is less explored. %24Jul2022EMW -- maybe also quickly toss in alpine systems and snowpack, if not here, then in discussion -- less snow generally advances phenology \citep[e.g.,][]{dunne2004,sherwood2017}
%24Jul2022EMW -- Maybe better to add here a paragraph on general 'challenges' beyond just observational? You could also use this to transition to temperate systems ... something like: Recently, however, recent studies have suggested that moisture [be careful with 'moisture' which can mean precip/snowpack/soil moisture and 'soil moisture' -- I mean moisture here] may play an  important---but complicated---role in the phenology of temperate ecosystem as climate change progresses \citep[e.g.,][]{seyed2018,wang2022}. \citet{wang2022} found that decreasing precipitation frequency correlates with earlier leafout in many regions, while others have variation across ecoregions \cite{seyed2018}. These studies, however, are observational where correlations between moisture and temperature are often high. Few experiments have directly reported on moisture effects in temperate systems [is this true? I am semi-making it up...], while effects in more arid systems are diverse, with often no overall shift in phenology \citep[e.g.,]{sherry2007,howell2020}.
...
%24Jul2022EMW -- before we get to analysis, we should more clearly state where and how we lose sites, and how many we end up with. I would also add HOW many species you end up with, and divide it across functional group and biome. 
\textbf {\emph{Analysis}}--
...
%24Jul2022EMW -- somewhere we should say something along the lines of. ... We used herarchical Bayesian models to test for effects for each species, as well as an overall effect, while accounting for site, year and plot-level effects. 
...
%24Jul2022EMW -- I would close or insert somewhere in the analysis section why we do not focus on treatment effects and instead use the microclimate data. 

\section* {RESULTS}
%24Jul2022EMW --  I would open with a paragraph the main effects for temp and moisture in standardized (and maybe natural units parathetically or insert those elsewhere in lead up to forecasting) and the interaction -- focusing on across species estimates (GDD models could be mentioned here in a sentence or two). Then I would have a paragraph on the species variability (potentially compare sigmas to show it matters a lot). Then in the same or next paragraph report functional groups and biomes, then close on a forecasting paragraph. 
%24Jul2022EMW -- Also, should we say soil drying or just 'lower soil moisture'?
...
\section* {DISCUSSION}
%24Jul2022EMW -- We should probably outline this before we dive in and try to write it. Some points to make ...
% Review the overall across species effects and discuss how these are temperate systems. 
% We looked at real soil moisture, which is hard to get -- most studies use precip or gridded moisture products which are well known to not be great ... but this was also a limitation as we ended up being able to use very few studies from the databases (cite the map figure?) 
% Something like: Lots of work on moisture from the perspective of semi-arid grasslands (including lots of experiments that find either none [cites] or contrasting results across species [cites]) and alpine systems dominated by snowpack, but our work shows that soil moisture importantly affect the phenology of more temperature systems ....
% Then put this in context, something like: Our finding of an overall effect where increased soil moisture leads to earlier budburst/leafout is in line with: \citet{seyed2018}, which found that moisture deficit generally delays phenology, temperate Mongolia grasslands \citet{tao2020} and a suite of studies that find this effect in some locations \citep{wang2022} or for some species \citep{tao2019}. 
% And mention co-limitation of temp and moisture?
% Maybe then transition to discuss species differences? We could suggest this is in line with other systems where effects of moisture do not appear consistent across species ... \cite{wolkovich2012} says that exotic species advance with precip; while natives delay at Fargo and discusses drought avoidance strategies at Konza (where results diverge but are less significant) ... connect to observational studies finding differences among ecoregions where species also differ (see end of doc, where I added some notes). Could stress our results mean we need to understand the drivers of these species-level differences better...
% Then I would have a section on your forecasting results and forecasting implications from this work. This is where the lack of effect across functional groups and biomes is most important I think. 
...
1) What do you think about adding GDD models for BB, LO, FL? %24Jul2022EMW -- seems good to add as back up to the main points!

2) What other figures do you think would strengthen/support the paper? %24Jul2022EMW -- a map! Maybe show all studies in the database and then highlight those with the data used here? I would also mv the current fig 3 (func. groups etc.) to supp 

3) Target Journal: American Journal of Botany? New Phytologist? GCB? Or somewhere else? %24Jul2022EMW -- start with GCB then move to New Phyt and AmJBot I suggest. 
...
%  \end{figure}
 ...
 %24Jul2022EMW -- this is a great figure! I would label the figure though: a) average species (or 'across species') then if you keep d move it up and label it d) Example average species (Quercus nigra); b) Super-additive (?) interaction (Carya glabra) and c) Subadditive interaction (Nyssa sylvatica) .. then add these terms to the caption and explain them if you use them *or* don't use them and instead try to think of short way to explain in figure. I think possibly easier in caption, see suggested edits below. 
 \begin{figure}[h]
\centering
