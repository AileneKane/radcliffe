...
The data reported in this paper are from the MC3E and the new ExPhen databases, which are both available at KNB \citep{ettinger2018,ettinger2022}. %2202Nov14EMW -- I added that ExPhen is new, because often editors/reviewers look more favorably on papers presenting new data ... and this is on the cover page. 
...
%2202Nov14EMW -- below -- I find 'on the other hand' too colloquial so would replace it but up to you (left it in for now) 
...
%2202Nov14EMW -- below -- do these experiments really OFTEN measure daily soil moisture and temperature? I might change that unless you really think it's true (and you would know better than I do). 
...
%2202Nov14EMW -- can you explain above how we get from 17+5 (22?) to 14 studies? Or, if it's there, tell me how I missed it and perhaps I can help adjust the text. 
%2023MarAE-- I added and corrected some numbers- hopefully this makes sense now- please let me know if more or something different is needed.
...
%2202Nov14EMW -- take or leave as useful (if you go with SD terminology, update results fully)
...
%2202Nov14EMW -- something is off in your numbers -- in the supp query to coauthors you mean 8K which sounds good to me, but here you only would have 4K iterations for analysis ... we REALLY should run 4 chains (how well can Rhat work on 2 chains?), so hopefully you did that here and just need to update? If not, can we switch to 4 chains and thus 8K samples?
%2202Nov14EMW -- Also, I use sampling iterations to mean post-warming but I see it's confusing (see https://discourse.mc-stan.org/t/draw-vs-sample-and-warmup-vs-sampling-iterations/158/2) so I deleted 'sampling' 
\section* {RESULTS}
%2202Nov14EMW -- some sections of the results are brilliant to me, but some feel like we're reading off stats not explaining the biology so I tried to change. it... see what you think. Also check my work on natural units and *please give the natural units* -- don't just refer to them as mysterious natural units. 

\par We found that both higher soil moisture and higher temperatures advance phenology, meaning two common effects of warming experiments---soil drying and warming---have contrasting effects on phenology. We found that soil drying delays phenology and warming temperatures advance phenology. For budburst, wetter soils and warmer temperatures alone advanced phenology by \Sexpr{soileff.bbcent} per SD of soil moisture  (or \Sexpr{soileff.bb/10} days per 10 percent increase in volumetric water content) and Sexpr{tempeff.bbcent} per SD of temperature (\Sexpr{tempeff.bb} per degree Celsius), respectively. Together, wetter and warmer conditions delayed phenology a small amount (interaction effect of \Sexpr{inteff.bbcent} per SD or \Sexpr{inteff.bb} natural units). 
\par The magnitude of soil moisture effects varied across phenophases, with effects on budburst being stronger than those on leafout (\Sexpr{soileff.locent} per SD of soil moisture) and flowering (\Sexpr{soileff.flcent} per SD). Similar to budburst, temperature effects were stronger than soil moisture for leafout (for which the temperature effect was \Sexpr{tempeff.locent} per SD) and flowering (for which it was \Sexpr{tempeff.flcent} per SD), across all species (Fig \ref{fig:bblofl}). 
%2202Nov14EMW -- below reads very statistic-y also, I wonder if we could just show the posteriors as a figure?
...
\par We applied the above budburst model to forecast possible effects of climate change on phenology. Based on the above estimated effects, wetter soils advanced spring budburst at a rate of 5 days per 10\percent increase in soil volumetric water content (VWC). Thus, if soil moisture is reduced by 10\% of its current state, as is expected over the next 50 years in areas where many of the experiments were conducted (the northeastern United States) \citep{berg2017} (moving from, e.g.,21.5\% VWC-- the mean value for January-March across all sites for which budburst was monitored-- to 19.4\%), budburst would be delayed by approximately 1 day on average, due to changes in soil moisture alone (Fig \ref{fig:bbsp}).
...
\section* {DISCUSSION}
\par Soil moisture has not been a focus of previous phenology meta-analyses \cite[e.g.,][]{wolkovich2012}, nor of most multi-species phenology studies in temperate mesic grasslands and forest ecosystems \cite[e.g.,][]{Vitass2021}.
We found that soil moisture can strongly affect phenology in temperate non-arid ecosystems, in addition to the arid ecosystems where effects of water availability on phenology have been more often reported (e.g., refs). Our findings highlight that, in mesic grasslands and forests, too, plants need water to advance budburst, leafout, and flowering; the delaying effect that dry soils seem to have suggests that moisture can be a hidden, but potentially limiting, factor affecting phenology temperate systems not typically thought to be water-stressed. 
%Although multiple environmental conditions affect phenology, we found interactive effects of soil moisture and temperature were weak for most phenophases. 
...
\par \bf High variation in species-level responses
\par Despite the overall effects we quantify of delays in phenology with soil drying, our results suggest that forecasts will need to contend with high variation in species responses. We do not find strong differences in soil moisture effects across functional types (Figure S2) but there may be traits associated with these species-level differences in soil moisture effects.   
