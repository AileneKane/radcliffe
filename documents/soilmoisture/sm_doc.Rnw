...
\par We found that soil drying delays phenology and warming temperatures advance phenology. For budburst, the soil moisture effect was \Sexpr{soileff.bbcent} standardized units (or \Sexpr{soileff.bb} natural units) and the temperature effect was \Sexpr{tempeff.bbcent} standardized units (\Sexpr{tempeff.bb} natural units), with interactive effects of \Sexpr{inteff.bbcent} standardized units (\Sexpr{inteff.bb} natural units). The magnitude of soil moisture effects varied across phenophases, with effects on budburst being stronger than those on leafout (\Sexpr{soileff.locent} standardized units) and flowering (\Sexpr{soileff.flcent} standardized units). Similar to budburst, temperature effects were stronger than soil moisture for leafout (for which the temperature effect was \Sexpr{tempeff.locent} standardized units) and flowering (for which it was \Sexpr{tempeff.flcent} standardized units), across all species (Fig \ref{fig:bblofl}). %GDD models suggest that soil moisture affects GDD for budburst, but not leafout or flower (Supp Fig, Table).%Temperature estimates are consistent with estimates from previous meta-analyses \citep{wolkovich2012}.
...
\section* {DISCUSSION}
%EMW30Aug2022 - Great stuff here! I think we just need to outline, re-arrange what text we have and add a little more. 
%\iffalse
\begin{enumerate}
