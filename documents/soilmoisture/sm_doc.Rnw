...
\section* {DISCUSSION}
\par  Across the life forms included in the ExPhen database (Table \ref{tab:studylocs}), soil moisture affects phenology of budburst, and  leaf-out. Soil moisture had previously been investigated primarily in grassland ecosystems (e.g. XXX), and has not been a focus of experiments and meta-analyses (e.g. Wolkovich). We quantify effects of soil moisture across forest and grassland ecosystems.
-Show estimated effects by manipulating posteriors by life form?
...
\centering
 \includegraphics{../../Analyses/soilmoisture/figures/m5.bbdlo.pdf}
 \caption{\textbf{Model coefficients from flowering and fruiting models (with centered predictors).}} 
 \label{fig:}
 \end{figure}
 \begin{figure}[h]
\centering
 \includegraphics{../../Analyses/soilmoisture/figures/bbmod_forecast_uncent_wCI.pdf}
 \caption{\textbf{Forecasting changes in soil moisture with warming.} instead of showing the mean choose 1-2 or more common species from each biome and show their differences.}
 \label{fig:bbforecast}
 \end{figure}
 %To do:
 %1. Figures
    % Make 
 %2. Reach out to Christy, Miriam and Jeff when the figures are done and be clear that 
 
 %3. Make it clear that this is pretty finished. 
