...
options(stringsAsFactors = FALSE)
 ...
 mod<-testm5cent.brms
tempeff.bbcent<-round(fixef(mod)[2,1], digits=1)
soileff.bbcent<-round(fixef(mod)[3,1], digits=1)
inteff.bbcent<-round(fixef(mod)[4,1], digits=1)
#add uncentered model
load("../../Analyses/output/brms/testm5.brms.bb.Rda")
 mod2<-testm5.brms
tempeff.bb<-round(fixef(mod2)[2,1], digits=1)
soileff.bb<-round(fixef(mod2)[3,1], digits=1)
inteff.bb<-round(fixef(mod2)[4,1], digits=1)
sdsoileff.bbcent<-round(summary(mod)$random$sp[3,1],digits=1)
sdtempeff.bbcent<-round(summary(mod)$random$sp[2,1],digits=1)


inteff.bbcent.lci95<-round(fixef(mod)[4,3], digits=1)
inteff.bbcent.uci95<-round(fixef(mod)[4,4], digits=1)

#leafout mod
load("../../Analyses/output/brms/testm5cent.brms.lo.Rda")
 lomod<-testm5cent.lod.brms
tempeff.locent<-round(fixef(lomod)[2,1], digits=1)
soileff.locent<-round(fixef(lomod)[3,1], digits=1)
inteff.locent<-round(fixef(lomod)[4,1], digits=1)
sdsoileff.locent<-round(summary(lomod)$random$sp[3,1],digits=1)
sdtempeff.locent<-round(summary(lomod)$random$sp[2,1],digits=1)

inteff.locent.lci95<-round(fixef(lomod)[4,3], digits=1)
inteff.locent.uci95<-round(fixef(lomod)[4,4], digits=1)

#flowering mod
load("../../Analyses/output/brms/testm5cent.brms.ff.Rda")
 flmod<-testm5cent.ffd.brms
tempeff.flcent<-round(fixef(flmod)[2,1], digits=1)
soileff.flcent<-round(fixef(flmod)[3,1], digits=1)
inteff.flcent<-round(fixef(flmod)[4,1], digits=1)
sdsoileff.flcent<-round(summary(flmod)$random$sp[3,1],digits=1)
sdtempeff.flcent<-round(summary(flmod)$random$sp[2,1],digits=1)

inteff.flcent.lci95<-round(fixef(flmod)[4,3], digits=1)
inteff.flcent.uci95<-round(fixef(flmod)[4,4], digits=1)
...
\section*{Abstract} 
%25Nov2024 -- I feel like we have two competing narratives (soil moisture is important and local context/complexity is important) in the abstract and we need to clearly elevate one for the reader and make the other subordinate. I suggest we focus on soil moisture is important, which means a few tweaks ... I started them below but you can take or leave them. 
Previous meta-analyses of phenology responses to climate change have focused largely on temperature as a driver of observed shifts. Yet climate change also affects soil moisture, which is limiting to many biological responses. Here we synthesize microclimate and phenology data from climate change experiments in temperate systems---both forests and grasslands---to quantify how soil moisture interacts with temperature to affect plant phenology. %25Nov2024 -- part before here is great. Nice simple declarative sentences and easy storyline to follow. 
We find that phenology (budburst, leafout and flowering) delays in drier soils, with the largest delays seen in budburst (\Sexpr{soileff.bb} days per percent reduction in soil VWC). %25Nov2024 -- this number in () is hard to take in and does not compare across events -- I would either remove it or replace it with something like effects of BB were xx% greater than flowering, and lowest in budburst (or whatever the finding was). 
While effects of soil moisture were much smaller than for temperature (\Sexpr{soileff.bbcent} versus \Sexpr{tempeff.bbcent} in standardized units), with interactive effects of temperature x moisture even smaller (\Sexpr{inteff.bbcent}), we found that these effects could have important implications. We found high variability in the response to shifted soil moisture across species; this combined with high variability in forecasts of future drought and precipitation in many systems may increase the importance of soil moisture to phenology in the future. Forecasting shifts in soil moisture with warming, we find that soil moisture declines of just 10\% would have important effects on the phenology of some species, potentially muting advances due to warming alone. %25Nov2024 -- very nice previous sentence, so I would be careful to lose this message with too much emphasis on complexity and local context (which, honestly, is a repeating theme in ecology -- most ecologists should know this). 
Our results show that soil moisture plays an important role in the phenology of temperate systems, with varying effects across species, and thus is likely to alter ecosystem functions tied to phenology.  %25Nov2024 -- I cut the below but think we can tweak it to fit back in. I like the focus on how this matters to management and conservation, but can we make a point that is not so focused on finer spatial scales, species complexity etc?  Maybe that as forecasts of precip improve we should be ready with the ecological knowledge to predict when these changes will matter to phenology and related services? 
%  at fine spatial scales. Incorporating local context, including relevant species and downscaled climate change projections, will be critical for planning appropriate management and conservation.
...
%25Nov2024 -- add a citation to end of first sentence?
...
%25Nov2024 -- can we tweak this last sentence to make the transition even better to the next paragraph? Something like: recent trends of advancing spring phenology have generally attributed this advance to warming springs---and potentially winters---with myriad studies focused on how the complexity of chilling and forcing together may determine spring phenology. 

%25Nov2024 --  internal organization of this paragraph could be slightly tighter ... maybe open with fundamental studies then move to climate change ones? 
...
Many climate change experiments have focused on the effects of altered precipitation regimes, though the role of soil moisture on phenology have been most well- quantified in arid and grassland ecosystems \citep[e.g., ][]{essiamah1986,reich1984, van1993, tao2019}. Effects in arid systems are diverse, often with no overall shift in phenology \citep[e.g.,][]{sherry2007,de2017challenging,howell2020}, suggesting that identifying clear trends from single experiments may be difficult Yet meta-analyses have also highlighted the diversity of findings \citep{lu2023contrasting}. Results in mesic regions have been equally diverse: \citet{wang2022} found that decreasing precipitation frequency correlates with earlier leafout in many regions, while others have found variation in moisture sensitivity across ecoregions \citep{seyed2018}. Increasing research using large-scale observational phenology data (e.g., remote sensing products such as NDVI) has documented an important role for soil moisture from forests to grasslands \citep{lian2020summer,shen2022plant,liu2024soil}, and suggested temperature may play a role through moderating soil moisture \citep{liu2024soil}. Teasing out the role of soil moisture from temperature is challenging through long-term climate trends alone, however. 
%25Nov2024 -- this paragraph feels a little repetitive with the previous one. I think we're probably trying too hard to fit in ALL the lit review (perhaps my fault). I made some big changes to try to fix this in the two paragraphs above, feel free to revert and we can discuss other ways to make it work (it was not bad, just did not feel tight enough to me). 
% \par Recent studies have suggested that moisture may play an important---but complicated---role in the phenology of temperate ecosystems as climate change progresses \citep[e.g.,][]{seyed2018,wang2022}.  ....Perhaps unsurprisingly then, many studies have attempted to manipulate moisture via experimental precipitation or drought treatments \citep[e.g.,][]{morin2010,hoeppner2012,rollinson2012b,clark2014a}, though few experiments have directly reported on moisture effects of phenology in temperate, non-arid and non-crop systems. 
...
%25Nov2024 -- methods read nicely!

\section* {RESULTS}
%25Nov2024 -- ooh, nice opening sentence!
\par We found that both higher soil moisture and higher temperatures advance phenology, meaning two common effects of warming experiments---soil drying and warming---have contrasting effects on phenology. We found that soil drying delays phenology and warming temperatures advance phenology. For budburst, wetter soils and warmer temperatures alone advanced phenology by \Sexpr{soileff.bbcent} per SD of soil moisture  (or \Sexpr{soileff.bb/10} days per 10 percent increase in volumetric water content) and \Sexpr{tempeff.bbcent} per SD of temperature (\Sexpr{tempeff.bb} per degree Celsius), respectively. We did not find evidence of strong interactive effects of soil moisture and temperature on phenology: together, wetter and warmer conditions delayed budburst only a small amount (interaction effect of \Sexpr{inteff.bbcent} per SD [95\% uncertainty intervals: \Sexpr{inteff.bbcent.lci95} to \Sexpr{inteff.bbcent.uci95}] or \Sexpr{inteff.bb} natural units). 
%25Nov2024 -- to discuss: how do you want to handle effects that cross 0? We mention it in the abstract but I wonder if we should not.... I also usually do 90% intervals also, since 95% is unstable and invokes p values a bit. 

%25Nov2024 -- I think we are missing the effect size for budburst below (only two effects reported for three total events)? Also, do you want to report the intervals around these estimates?
\par The magnitude of soil moisture effects varied across phenophases, with effects on budburst being stronger than those on leafout (\Sexpr{soileff.locent} per SD of soil moisture) and flowering (\Sexpr{soileff.flcent} per SD). Similar to budburst, temperature effects were stronger than soil moisture for leafout (for which the temperature effect was \Sexpr{tempeff.locent} per SD) and flowering (for which it was \Sexpr{tempeff.flcent} per SD), across all species (Fig \ref{fig:bblofl}). Estimates of interactions between soil moisture and temperature on phenology also varied by phenophase, with weak positive interactive effects estimated for leafout (\Sexpr{inteff.locent}) and budburst (\Sexpr{inteff.bbcent}) and a stronger but negative the interaction for flowering (\Sexpr{inteff.flcent}). This negative interaction implies that there may be synergistic effects of soil moisture and temperature (both of which also have negative estimated effects on flowering), resulting in flowering that advances even more strongly than would be expected by simply adding together the  estimated effects of temperature and  moisture each acting alone. 

%25Nov2024 -- we may need to add some more means and intervals for the below? Or, reference a table in the supp with the test statistics. 
\par These overall effects varied widely across species (Fig \ref{fig:bblofl}). Species-level variance for the effect of moisture was \Sexpr{sdsoileff.bbcent} standardized units for budburst, \Sexpr{sdsoileff.locent} for leafout, and \Sexpr{sdsoileff.flcent} for flowering. Species-level variance was even greater for temperature effects: \Sexpr{sdtempeff.bbcent} for budburst, \Sexpr{sdtempeff.locent} for leafout, and \Sexpr{sdtempeff.flcent} for flowering. 
...
\section* {DISCUSSION}
%25Nov2024 -- possible to mention \citep{zhou2023climate} below? Not sure if it fits, happy to discuss when we talk!
\par Synthesizing plot-level climate change experiments, we found that soil moisture affects plant phenology in temperate non-arid ecosystems, in addition to the arid ecosystems where effects of water availability on phenology have been more often reported \cite[e.g.,][]{reich1984,van1993,cleverly2016soil,bertiller1991seasonal}. %25Nov2024 -- Below, can you say what new insights or re-work somehow? Could be something like: our results suggest previous hypotheses that soil moisture effects are mostly in arid ecosystems may not be supported by synthetic research across experimental sites? Though I don't love that sentence either ... Or, you could cut/move the below two sentences, which I am not sure are critical here -- they are more rah-rah on the method, which I think fits better later -- stay focused on the biology here. 
This offers new insights because there has been little synthetic research across experimental sites to understand impacts of soil moisture on phenology, despite the reality that many experiments collect these data. Soil moisture has not been a focus of previous phenology meta-analyses \cite[e.g.,][]{wolkovich2012}, nor of most multi-species phenology studies in temperate mesic grasslands and forest ecosystems \cite[e.g.,][]{Vitass2021}, 
...
%25Nov2024 -- suggest rewriting to avoid word 'to get' which seems informal to me, though I am all for language changing if you think it works!
%25Nov2024 -- also suggest you sneak in a little more on moisture forecasts here -- we used 10% but you should explain somewhere in the discussion that this could be too big a change or way too small: forecasts are very uncertain, but if effects are bigger then what we found is bigger...
%25Nov2024 -- also, after finishing the discussion I felt like the forecasting was undersold -- it's a cool part of the paper and something few papers do at the same time of the analyses .. I suggest building this section up a little to a couple paragraphs with possibly its own subheader (if you want to do subheaders)
...
%25Nov2024 -- feels like you have a section on species/event responses between the below three paragraphs and the one about how these scale to ecosystem services?
...
%25Nov2024 -- suggest we cut below sentence.
Such shifts are likely to affect fitness, as well, with some species more strongly affected than othersm thus scaling up to impact community structure and function, as \citep{buonaiuto2021differences} also suggest.
...
%25Nov2024 -- I think the below two paragraphs form sort of a forecasting next steps section, which is nice to end on. 
%25Nov2024 -- possible to mention \citep{zhou2023climate} below? Not sure if it fits, happy to discuss when we talk!
...
%25Nov2024  -- suggested we cut below paragraph.
...
%25Nov2024  -- I like the below point and think you could move down some of the sentences related to synthesis from start of discussion to here perhaps 
...
%25Nov2024  -- I am a little unsure of the order of the discussion ... it doesn't jump out at me as problematic, but it also does not flow perfectly. I think the below paragraph should probably go up (ending on previous paragraph would be good, also). Perhaps it could fit somewhere around traits or around flower versus leaf stuff? Either way, I suggest spending a short while (an hour?) looking at order of discussion and making sure it's pretty good. I could do this myself if you prefer. And... I just added some suggested header spots that might help. See what you think. 
...
\centering
 ...
 \caption{\textbf{Model coefficients from budburst, leafout, and flowering models (with centered predictors)} and including all species. We could show only the most common species here, to improve readability, and then show this version (with full species list) in the supplement. Thoughts?}  %25Nov2024  -- I like the idea of fewer species here and ref full figure in supp.
 \label{fig:bblofl}
 \end{figure}
