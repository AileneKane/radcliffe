...
moismod2<-lmer(soilmois_perc~-1+type2*target + (1|site/year/doy), data=expclimt, REML=FALSE)
...
rownames(moistargtable)<-c("intercept (infrared)","intercept (forced air)","target (infrared)","target (forced air)")
