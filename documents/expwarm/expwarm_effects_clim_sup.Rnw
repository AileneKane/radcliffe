...
\begin{figure}[p]
\centering
\includegraphics{/Users/aileneettinger/git/radcliffe/Analyses/figures/targetvsobs_stduration
.pdf} 
\caption{\textbf{Mean warming across the study duration.}Reporting only the mean temperature difference across the duration of the study, as is commonly done in publications of climate change experiments, hides potentially important variations in daily, seasonal, and annual temperatures among treatments. Compare the above figure to Figure } 
