...
We searched the Web of Science (ISI) for Topic=(warm* OR temperature*) AND Topic=(plant* AND phenolog*) AND Topic=(experiment* OR manip*). We restricted dates to the time period after the STONE database (i.e. January 2011 through March 2015). This yielded 277 new studies. We removed all passive warming studies from the list.  .%should we include this last sentence or not? it seems a bit tattle-tale-y but it is in response to a reviewer comment from NCC. Yann's comment:shall we really point out who were reluctant to share the data... or only say how many dataset we've got?
% In addition, a secondary goal of this database was to test hypotheses about mechanisms for the mismatch in sensitivities between observational and experimental phenological studies \citep{wolkovich2012}. Because of this secondary goal, studies included in the database had to either 1) include more than one level of warming, or 2) manipulate both temperature and precipitation. (Some studies met both of these criteria.) 

\section* {Details of Statistical Analyses and Results}
% EMW: Explain basics of mixed-models and Wald II versus III here
% Also, check use of VWC and use it consistently across tables (percent VWC? Define or not etc. ... just keep the styles matched when possible). 
...
% EMW: This sentence:
% We first fit a model with a fixed, continuous effect of target warming level (this ranged from 1.0 to 5.2 \degree C for these 10 studies).
% Is not totally clear -- I think you fit a separate model to each study? If so, make that clear. 
...
% EMW: It's hard to tell what is unique of the below compared to the above. I think you just need a slight change to the topic sentence ... In addition to testing how experimental warming influenced soil moisture, we also tested how experimental structures influenced soil moisture. [Also, what was previous analysis? Above? I don't fully understand the first sentence.]
