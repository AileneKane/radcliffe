...
In the most recent round of comments, Reviewer 1 felt that our manuscript offered an important contribution to the literature and suggested no changes. Reviewer 2 felt that two main issues required revision: 1) variation among experimental warming techniques (specifically feedback versus constant wattage approaches) was not made clear enough; and 2) ``indirect effects" were presented in a biased way.  We have modified the main text and figures, and added a new box focused on warming techniques to address these concerns. Please see below for details on our revisions.  % I think you need to make the second to last sentence here stronger: how many figures did you change? You should add if you made text changes to all sections ... for example: We have extensively modified the manuscript to address these concerns: making changes to X/6 figures, changing the abstract and conclusion and making changes to all section of the main text. We also have added a new box focused on warming techniques to address these concerns, which we reference throughout the manuscript..
...
% I think you are being too unclear here, I adjusted the text but if you prefer your reply feel free to change back.
...
\par Unfortunately, no infrared studies included both ambient and structural controls, so we were unable to include them in the analysis. We do discuss in this section a separate analysis of infrared studies, which is suggestive of microclimate effects from infrastructure, likely via shading, which has been documented in other studies (see Table 1). Thus, because we discuss three distinct warming types (forced air, soil cable, and infrared) it would be inaccurate to have a heading of `Chamber (forced hot air) heating alters microclimate.' To address the reviewer's concerns, we considered changing the heading to `Forced hot air and soil cable infrastructure alters microclimate (infrared infrastructure untested).' This heading, however, seemed long and cumbersome, and infers that other designs do not have these issues, when instead we do not have data to assess these issues in other designs, which we state on lines XX-XX and XX-XX. % Finish last sentence. 
...
\end{quote}
% I think this is quite good! You might only cite the Cook paper and say 'we have now cited this paper to clarify ...' or such. 
