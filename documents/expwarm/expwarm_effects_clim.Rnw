...
\title{How do climate change experiments alter microclimate?}
...
\par We were unable to compare ambient and structural controls for infrared heating, because no studies in our database included both control types. We were able to compare differences between the two control types and the measured amount of warming per degree of target warming, across seven infrared sites that measured soil temperature. Of these seven sites, two used ambient controls (exp06 and exp11) and five used structural controls (exp1,exp09,exp12, exp13, exp14). Among these studies, warming per \degree C of target warming was close to one for structural controls (0.89\degree C, SE= 0.05, Table SX), but was significantly lower for ambient controls (0.30 \degree C of warming per \degree C of target warming, SE=0.08, Table SX). The number of studies using ambient controls is small (n=2), but this trend is suggests that infrared heating equipment alone alter microclimate (e.g., by shading).
