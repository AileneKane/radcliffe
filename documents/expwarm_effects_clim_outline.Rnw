...
\caption{Difference between min and max temperature in sham controls compared with ambient controls, with no sham chambers/warming infrastructure. Fixed effects from the a mixed effects model are plotted in the above graph (see shamvambient.R for details). Ben suggests: make the ranges on the y-axes all the same, centered around zero, and add a dashed line at zero. This would make them more intuitive and easier for the reader to see the differences}
\end{figure}
...
\begin{itemize}
\item Effects of experimental warming on soil moisture (Figure 3, and Miriam's analysis). (this seems obvious but it is often overlooked or glossed over. for example, the soil warming experiments at harvard they just write one sentence about the change in moisture in each paper- this is from yann- does he mean clark et al?)
\begin{figure}[p]
    \centering
...
in climate variables. can use time-series modeling, not just monthly (or even daily) means. Get the ACF and PACF
...
\item 
\end{itemize}
Notes from Aaron, who doesn't think that pooling the plots makes sense. (I should explain that I used mixed effects models with random effect of site to account for this):
There are some big differences in exps that are obscured by pooling in these plots. Because of that, pooling them doesn't make sense to me.
1. Farnsworth is soil-warming cables only
2. Clarkduke & Clarkharvard are soil warming cables + air heating, but air heating is turned off in winter at Harvard (not sure about Duke) while cables (I think) were left on.

3. ellison and marchin are air warming only, run all year.
