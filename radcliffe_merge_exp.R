...
### By Ailene Ettinger ###
setwd("~/Dropbox/Documents/Work/Wolkovich/Radcliffe/Google Drive/experiments")
...
#1. Phenology data: site, plot, event (phen event),year, doy,genus,species,Temp_treat	Precip_treat	Year	Genus	Species		onetempchange	precipchange	precipchangetyp)
...
###Marchin et al. frmo Duke
head(marchin_duke1)
## clean.raw$marchin <- function(filename="Budburst_Marchin.csv", path=".",
#names.only=FALSE) {
  
#  return(marchinphen)
#}
  marchin1<-read.csv("Budburst_Marchin.csv", header=T)
  event<-rep("BB", times=dim(marchin1)[1])
  marchin<-cbind(marchin1$Year,marchin1$Species,marchin1$Chamber,event,marchin1$Budburst_JD)
  colnames(marchin)<-c("year","species","plot","event","doy")
  marchin2<-read.csv("Flower_Marchin.csv", header=T)
  event<-rep("FF", times=dim(marchin2)[1])
...
clean.raw$konza <- function(filename="Konza_PlantPhenology.csv", path=".",
    ...
    names(konza1)[3] <- "observer"
    names(konza1)[4] <- "plot"
    names(konza1)[14] <- "comments"
    names(konza1)[15] <- "planttype"
    konza2 <- subset(konza1, select=c("common name","genusspecies","2009",
